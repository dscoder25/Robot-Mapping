...
for i = 1:numParticles
  ...
  for j = 1:m
    ...
    if (particles(i).landmarks(l).observed == false)
      ...
      particles(i).landmarks(l).mu = [(robot(1)+ z(j).range*cos(robot(3)+z(j).bearing));(robot(2)+ z(j).range*sin(robot(3)+z(j).bearing))];
      ...
      err = [z(j).range - expectedZ(1);normalize_angle(z(j).bearing - expectedZ(2))];
      % TODO: update the mean and covariance of the EKF for this landmark
      particles(i).landmarks(l).mu = particles(i).landmarks(l).mu + K*err;
      int_st = K*H;
      particles(i).landmarks(l).sigma = (eye(size(int_st))- int_st)*particles(i).landmarks(l).sigma;
      % TODO: compute the likelihood of this observation, multiply with the former weight
      %       to account for observing several features in one time step
      particles(i).weight = particles(i).weight * (1/sqrt(2*pi*det(Q)))*exp(-0.5*(err)'*inv(Q)*(err));
    end
