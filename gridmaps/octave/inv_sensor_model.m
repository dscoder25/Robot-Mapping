...
for sc=1:columns(laserEndPntsMapFrame)
        ...
        pt = laserEndPntsMapFrame(:,sc);
        [X,Y] = bresenham(map,[robPoseMapFrame(1),robPoseMapFrame(2);pt(1),pt(2)]);
...
l_free = prob_to_log_odds(probFree);
l_occ = prob_to_log_odds(probOcc);
disp(size(freeCells));
%TODO: update the log odds values in mapUpdate for each free cell according to probFree.
  for i = 1:columns(freeCells)
    ...
    mapUpdate(rows(map)-y,columns(map)-x)= mapUpdate(y,x)+l_free;
  endfor
...
%TODO: update the log odds values in mapUpdate for each laser endpoint according to probOcc.
  for i = 1:columns(laserEndPntsMapFrame)
    ...
    mapUpdate(rows(map)-y,columns(map)-x)= mapUpdate(y,x)+l_occ;
  endfor
